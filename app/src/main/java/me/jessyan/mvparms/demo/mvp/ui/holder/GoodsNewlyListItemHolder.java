 ...
 * ================================================
 */
public class GoodsNewlyListItemHolder extends BaseHolder<Goods> {
    ...
    MoneyView salePriceMV;
    @BindView(R.id.sale)
    TextView saleTV;
    @BindView(R.id.buy)
    View buyV;

    private GoodsNewlyListAdapter.OnChildItemClickLinstener onChildItemClickLinstener;

    private AppComponent mAppComponent;
    private ImageLoader mImageLoader;//用于加载图片的管理类,默认使用 Glide,使用策略模式,可替换框架

    public GoodsNewlyListItemHolder(View itemView, GoodsNewlyListAdapter.OnChildItemClickLinstener onChildItemClickLinstener) {
        super(itemView);
    ...
    public void setData(Goods goods, int position) {
        ...
        Observable.just(goods.getVipPrice())
                .subscribe(s -> priceTV.setMoneyText(String.valueOf(s)));
        Observable.just(goods.getSalePrice())
                .subscribe(s -> salePriceMV.setMoneyText(String.valueOf(s)));
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.buy:
                onChildItemClickLinstener.onChildItemClick(view, ViewName.BUY, getAdapterPosition());
                return;
        }
